...
#    self.scenario = [(self.Exposition, 1.0)]
  ...
  def execute_it_in_the_thread(self):
...
#         print('\n\n\n 2222 Set my erreor True \n\n\n')
         ...
         if self.step_name.upper() == 'EXPOSE':
             mainApp.expose_frame.sci_scenario_table.next()
# lll
       except Exception as e:
         print('Exception: res =', res, type(res))
...
#      self.end_of_scenario()
  ...
  def do_it_when_file_is_ready(self):
...
# Fix filename here BEFORE new exposure start !!!!!!!!!!!!!!!!
    fixed_file_name = nbimodule.FILENAME_tkvar.get()
    fixed_file_path = nbi.get_filepath()
    fullpath = os.path.join(fixed_file_path, fixed_file_name)
     

# !!! Run new Thread here to handle ds9 view etc Move that all into the new thread!!!!
    ...
    printd ('Send2ImageServer')
    if fixed_file_name == 'autofocusing.fits':
      printd('+++++++++++++ AUTOFOCUS FILE is ready ++++++++++++++++++++++')
      imageserver.sendFileReadyFocus(fullpath, focuser.foc_start.get(), focuser.foc_step.get(), \
               focuser.nsteps.get(), DERO.get(), PARANG.get())
    elif mainApp.ocs.iq.get() == 1:
      imageserver.sendFileReadyIQ(fullpath)
    if mainApp.ocs.ds9view.get() == 1:
         printd ('\nsds9view.get()=', mainApp.ocs.ds9view.get())
         ds9nbi.view(fullpath)
    if mainApp.ocs.bd.get() == 1:
        upload2bd.upload(fixed_file_name, fixed_file_path)
...
#anchor=Tk.W, side=Tk.LEFT, fill=Tk.BOTH, pady=pdy, padx=12
    ...
    self.quitCCD3   = Tk.Button(frame2, text="QuitCCD3", highlightbackground='blue', command=self.quitCCD3)
    self.quitCCD3.pack(side = Tk.LEFT, padx = 5, pady = 2)

    self.initCCD3   = Tk.Button(frame2, text="InitCCD3", highlightbackground='blue', command=self.initCCD3)
